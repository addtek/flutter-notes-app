...
import 'package:QuickNotes/Foundation/Provider/NotesProvider.dart';
...
class NotesBloc {
  ...
  void deleteSavedNote(int id) async {
   var currentNotes = _notes.value;
    if (currentNotes.any((n) => n.id == id)) {
  ...
  Future loadInitialNotes() async {
    ...
    if (notes.length > 0) {
      var highestId = Collection<Note>(notes).max$1((Note n) => n.id).toInt();
